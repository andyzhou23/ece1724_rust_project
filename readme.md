...
This need for quick, public and unrestricted access is why we decided to develop a light-weight, real-time, instant online chat solution. Our application allows anyone to create a chat group instantly by generating a join code, enabling participants to join quickly by simpy entering the join code without permissions or resitrictions and leave the group freely.
...
When receiving a `ClientMessage` message, the `ChatServer` actor will first check if the user is a member of the group, if not, return an error message. Then, it will insert the message into dthe atabase and broadcast it to all online members in the same group by sending a `BroadcastMessage` message to each matched `ConnectionActor`.
...
7. **Leave a group**: Click the cross button on the group card in the main page to leave the group.
